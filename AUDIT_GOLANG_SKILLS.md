...
| S5 | Med | internal/server/http.go:108,110, docker-compose.yml:9 | Binds `:PORT` (all interfaces); compose publishes `0.0.0.0:3000` — unauthenticated service reachable from the LAN | HOST env defaulting to loopback; pin compose publish IP |
...
| S7 | Med | internal/ingest/fetcher.go:142 (also ingest.go:322-323) | Scraper reads response bodies with unbounded `io.ReadAll` — hostile/compromised origin OOMs the pipeline | `io.LimitReader` with max size |
...
| E7 | Med | internal/ingest/discovery.go:296 (writeJSONFile → os.WriteFile), ingest.go:437 (HTML cache), 446,464,471 (seeds) | Seeds, HTML cache and discovery cache written non-atomically — interrupt leaves truncated files; truncated seed then blocks `--resume` (E6); truncated HTML silently re-parses to METADATA_ONLY under `--skip-fetch` | Write temp file + rename in same dir |
...
# Fact-check pass (2026-08-27, post-commit `610007a`)

Method: `codebase-memory-mcp` re-indexed in full mode against the `go-port` tree (63,969 nodes / 73,474 edges; 0 skipped, 0 parse-partial — complete Go coverage). ~45 of the highest-impact claims were then re-verified directly against the graph and source (12 full-function snippets, symbol/line lookups, regex absence sweeps); the remaining findings stand on the original auditors' full-file reads. Non-Go claims (CI YAML, shell scripts, project docs) were re-checked by direct read.

Result: **all findings confirmed**; four citation-level corrections applied to the tables above:

- S7: unbounded `io.ReadAll` is at `internal/ingest/fetcher.go:142` (`resp.Body.Close()` is :143).
- E7: seed files are written via `writeJSONFile` (→ `os.WriteFile`, discovery.go:296) at `ingest.go:446, 464, 471` — not :442/462/471; the HTML cache write at :437 is a direct `os.WriteFile`.
- S5: the all-interfaces bind is `http.Server{Addr: ":" + port}` at http.go:108, with `net.Listen` at :110.
- T6: the `RealDBAvailable()` guards are at store_test.go:276, get_provision_test.go:174, search_legislation_test.go:242 (the `t.Skip` calls follow one line later).

Exact-match confirmations (spot list): `dispatch` discards ctx (`_ context.Context`) and returns raw `err.Error()` at registry.go:243, with no `recover()` and the duplicated "About tool not configured." literal at :232; `http.Server` at http.go:108 has no timeouts; `mcp.NewStreamableHTTPHandler(func(*http.Request) *mcp.Server { return sessionServer(db, about) }, …)` rebuilds the full server per request (http.go:218-220, `ponytail:` comment :221-225); health success cached / failures re-probe the full COUNT pair under `healthMu` (http.go:234-253); `Build` deletes the existing DB before validating seeds (build.go:68-74) and drops every `insertEuReference` error (`if … err == nil { totalEuReferences++ }`, build.go:229-232, TS-parity comment in place) and silently skips a missing `eu-mappings.json` (no log in the `fs.ErrNotExist` branch); `FetchAndParseActs` unconditionally returns nil (ingest.go:554) after the `p.printf("")` no-op (:553) and aborts `--resume` on one bad seed (:402); `page <= totalPages` unbounded crawl at discovery.go:249; discovery cache written only at :276; zero `%w` in internal/tools; zero `QueryContext`/`QueryRowContext` anywhere; `SetMaxOpenConns(1)` only in builddb/tests — never on the server's read-only handle (store.go:30-32); zero `MaxBytesReader`/`LimitReader`/`ReadHeaderTimeout`/`IdleTimeout`/`busy_timeout`/`Retry-After` repo-wide; `recover()` only at http.go:292 and its log drops the stack; `signal.NotifyContext` only in stdio.go:40 (cmd/ingest/main.go:45 uses bare `context.Background()`); `rows.Err()` absent exactly in capabilities.go/metadata.go, with metadata.go:51 caching the possibly-degraded result unconditionally; capitalized error strings at dbinfo.go:40, about.go:83, prompts.go:53 (parity text pinned by store_test.go:182 and registry_test.go:59); `Eu`/`EU` and `Db`/`DB` acronym splits, `ResolveDocumentId` (statute.go:63), `dbIdWithSecRe` (validate_citation.go:52), `const SCHEMA` (schema.go:11), `fts.SanitizeFtsInput` (fts.go:38; case-sensitive `booleanOpsRe` at :13); modernize sites exact (sort.Ints ingest.go:259; sort.Slice discovery.go:274 + parser.go:447; math.Min search-eu-implementations.go:55; boolPtr registry.go:187-193; os.IsNotExist cmd/check-updates/main.go:106); exactly 9 `missing required argument` guards and 18 `context.Background()` test sites; the 9 nameless table loops (parser_test.go :75/:97/:131/:146 verified by read); `internal/server` = exactly 5 files, no tests; package doc comments attached to the package clause in server.go:1-3 and registry.go:1-3; capabilities.go:57 identifier interpolation (documented in its own doc comment as TS parity); check_currency.go:62-64 lacks the `sql.ErrNoRows` branch its siblings have; dependabot npm-only, all five triggered workflows `branches: [main]` only, semgrep `p/typescript` at :29, `setup-go@v5`/`go-version: '1.26'` in ci.yml, entrypoint stdout echoes at :22/:54, download-db.sh cross-org download without checksum and `set -e` without `pipefail`; `serverVersion = "1.0.0"` (server.go:17) vs CHANGELOG `[2.0.0]`; CHANGELOG:8 "health/version routes" claim (no `/version` route exists in the handler's route set) and DISCLAIMER:78 ">30 days" staleness-warning claim (no such feature; only `built_at` metadata and check-updates' 90-day threshold); README.md:85 "Uniós kereszthivatkozások | 109" confirmed verbatim.
